...
static uint8_t layers_cache[(MATRIX_ROWS * MATRIX_COLS + 7) / 8][MAX_LAYER_BITS];

static void update_layers_cache(keypos_t key, uint8_t layer);
static uint8_t read_layers_cache(keypos_t key);
...
static uint8_t read_layers_cache(keypos_t key) {
    ...
    uint8_t layer = 0;
    for (int8_t column = MAX_LAYER_BITS - 1; column >= 0; --column) {
        if (layers_cache[row][column] & mask) layer |= _BV(column);
    }
    return layer;
}

uint8_t find_layer(keypos_t key) {
 ...
 */
uint8_t get_layer(keypos_t key, bool pressed) {
    if (pressed) {
        uint8_t layer = find_layer(key);
        update_layers_cache(key, layer);
        return layer;
    }
    return read_layers_cache(key);
}
